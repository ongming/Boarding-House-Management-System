...
public class StaffHomePanel {
    ...
    private Label legendChip(String text, String bg, String fg) {
        ...
        label.setStyle("-fx-background-color:" + bg + "; -fx-text-fill:" + fg + "; -fx-background-radius:999; -fx-padding:6 12; -fx-font-weight:bold;");
    ...
    private HBox buildFloorButtons() {
        ...
        for (Integer floor : catalog.getFloors()) {
            ...
            btn.selectedProperty().addListener((obs, oldV, selected) -> btn.setStyle(selected ? selectedFloorStyle() : normalFloorStyle()));
    ...
    private BorderPane buildContent() {
        ...
        box.setStyle("-fx-background-color:white; -fx-padding:16; -fx-background-radius:12; -fx-effect:dropshadow(three-pass-box, rgba(0,0,0,0.08), 12, 0, 0, 3);");
        return new BorderPane(box);
    }
    private VBox buildDetail() {
        detailBox.setPrefWidth(320);
        detailBox.setPadding(new Insets(16));
        detailBox.setStyle("-fx-background-color:white; -fx-background-radius:12; -fx-effect:dropshadow(three-pass-box, rgba(0,0,0,0.08), 12, 0, 0, 3);");
        showDetail(null);
        return detailBox;
    }
    private void selectFloor(int floor) {
        ...
        grid.getChildren().setAll(filteredRooms.stream().map(this::buildCard).toList());
        if (!filteredRooms.isEmpty()) {
            selectRoom(filteredRooms.getFirst());
        } else {
            showDetail(null);
        }
    }
    private StackPane buildCard(StaffRoomSnapshot room) {
        ...
        wrapper.setOnMouseExited(e -> card.setStyle(cardStyle(room, selectedRoom != null && selectedRoom.roomNumber().equals(room.roomNumber()))));
    ...
    private void selectRoom(StaffRoomSnapshot room) {
        ...
        grid.getChildren().forEach(node -> {
            if (node instanceof StackPane stack && stack.getChildren().getFirst() instanceof VBox card) {
                Label title = (Label) card.getChildren().getFirst();
                boolean selected = title.getText().equals("Phòng " + room.roomNumber());
                card.setStyle(cardStyle(room, selected));
            }
        });
    }
    private void showDetail(StaffRoomSnapshot room) {
        ...
        Button contract = new Button(room.canCreateContract() && onCreateContract != null ? "Lập hợp đồng" : "Không thể lập hợp đồng");
        ...
        contract.setStyle(canCreateContract
                ? "-fx-background-color: linear-gradient(to right, #22c55e, #16a34a); -fx-text-fill:white; -fx-font-weight:bold; -fx-background-radius:8; -fx-padding:10;"
                : "-fx-background-color: #cbd5e1; -fx-text-fill:#475569; -fx-font-weight:bold; -fx-background-radius:8; -fx-padding:10;");
        contract.setOnAction(e -> { if (canCreateContract) onCreateContract.accept(room.roomNumber()); });
    ...
    private String cardStyle(StaffRoomSnapshot room, boolean selected) {
        ...
        return "-fx-background-color:" + bg + "; -fx-border-color:" + border + "; -fx-border-width:1; -fx-border-radius:14; -fx-background-radius:14; -fx-effect:dropshadow(three-pass-box, rgba(0,0,0,0.08), 10, 0, 0, 2);";
    }
    private String normalFloorStyle() {
        return "-fx-background-color:white; -fx-border-color:#d1d5db; -fx-border-radius:999; -fx-background-radius:999; -fx-text-fill:#1f2937; -fx-font-weight:bold; -fx-padding:8 14;";
    }
    private String selectedFloorStyle() {
        return "-fx-background-color: linear-gradient(to right, #2563eb, #1d4ed8); -fx-border-radius:999; -fx-background-radius:999; -fx-text-fill:white; -fx-font-weight:bold; -fx-padding:8 14;";
    }
    private String formatMoney(double value) { return String.format("%,.0f", value); }
}
