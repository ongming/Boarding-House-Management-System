...
import com.example.house.service.impl.StaffWorkflowServiceImpl;
...
public class JpaStaffDataStore implements StaffDataStore {
    private final StaffWorkflowService workflowService;
    ...
    public JpaStaffDataStore() {
        this(new StaffWorkflowServiceImpl());
    }

    public JpaStaffDataStore(StaffWorkflowService workflowService) {
    ...
    public ContractItem addContract(String roomCode, String tenantName, double rentFee, double deposit) {
        Contract contract = workflowService.processContractCreation(new com.example.house.service.staff.ContractCreationRequest(
                roomCode,
                tenantName,
    ...
    public InvoiceItem markInvoicePaid(int invoiceId, String paymentMethod) {
        InvoicePaymentMethod method = "TIEN_MAT".equalsIgnoreCase(paymentMethod) || "Tiền mặt".equalsIgnoreCase(paymentMethod)
    ...
    private ContractItem toContractItem(Contract contract) {
        ...
        return new ContractItem(contract.getId(), roomCode, tenantName, roomFee, deposit, LocalDateTime.now());
    }
